...
%directory = 'E:\data\model\Combination\';
...
%% modified by Qiannan 23/6/8
IEX_CO2={'Pluralibacter_IEX_cpd00011[u]tr','Shewanella_IEX_cpd00011[u]tr','Vibrio_IEX_cpd00011[u]tr','Aeromonas_IEX_cpd00011[u]tr'}
for i = 1:length(models) %i=8
		models(i).COBRAmodel = changeRxnBounds(models(i).COBRAmodel, IEX_CO2, 0, 'l');
		solution = optimizeCbModel(models(i).COBRAmodel);
end
IEX_O2={'Pluralibacter_IEX_cpd00007[u]tr','Shewanella_IEX_cpd00007[u]tr','Vibrio_IEX_cpd00007[u]tr','Aeromonas_IEX_cpd00007[u]tr'}
for i = 1:length(models) %i=8
		models(i).COBRAmodel = changeRxnBounds(models(i).COBRAmodel, IEX_O2, 0, 'u');
		solution = optimizeCbModel(models(i).COBRAmodel);
end
...
for i = 1:length(models) %i=8
	if (i == 1)
		...
		if(solution.stat == 1)
            ...
            bioIDs = find(contains(models(i).COBRAmodel.rxns,'bio1')) %bioIDs = find(models(i).COBRAmodel.c);
				tempnum=strmatch(strcat(models(i).Name,'.mat'),filenames);
				BioAndDegradation{k,2+tempnum} = solution.x(bioIDs);
			%%
			...
			models(i).COBRAmodel.c(temIDs) = 1;%Set the objective function, take the degradation amount of the target substrate as the objective function, and find the minimum value.
		...
		solution = optimizeCbModel(models(i).COBRAmodel);
        %%%FVA i=8 modified by Qiannan 23/5/15
        %[minF, maxF] = fluxVariability(models(i).COBRAmodel, 100);
        %FVA=[minF maxF];
        %%%
		if(solution.stat == 1)
			%%%print resluts%%%	
			BioAndDegradation{k,2} = solution.f;	
			bioIDs = find(contains(models(i).COBRAmodel.rxns,'bio1')) %bioIDs = find(models(i).COBRAmodel.c);
			for m = 1:(length(bioIDs)-1) %m = 1:length(bioIDs)
