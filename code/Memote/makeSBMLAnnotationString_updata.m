function [annotationString,notes] = makeSBMLAnnotationString_updata(model,id,fieldentries,position,mergeAnnotations)
...
for pos = 1:size(fieldentries,1)        
    ...
    fieldMappings = getDatabaseMappings_update(field);
    [~,upos,~] = unique(fieldMappings(:,3));
    fieldMappings = fieldMappings(upos,:);
    fieldMappings(13, 1) = {'seed.reaction'}; % origi({'seed.reactions'})
