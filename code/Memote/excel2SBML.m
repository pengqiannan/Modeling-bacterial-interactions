...
sbmlModel = writeSBML_update(model, 'P.gergoviae');
...
sbmlModel = writeSBML_update(model, 'V.alginolyticus');
...
model.rxnSEEDID=table2cell(rxn_list(:,11));
%% add rxnRheaID
model.rxnRheaID = table2cell(rxn_list(:,12));
%% add MetaNETID
model.rxnMetaNetXID= table2cell(rxn_list(:,13));
%% add rxnBiggID
model.rxnBiGGID=table2cell(rxn_list(:,14));
...
sbmlModel = writeSBML_update(model, 'A.hydrophila');
% outmodel = writeCbModel(model, 'format','mat');
...
sbmlModel = writeSBML_update(model, 'S.putrefaciens');
% outmodel = writeCbModel(model, 'format','mat');
