function sbmlModel = writeSBML_update(model,fileName,compSymbolList,compNameList)
...
for i=1:size(model.mets, 1)
    ...
    [tmp_annot,met_notes] = makeSBMLAnnotationString_updata(model,tmp_species.metaid,'met',i);
...
for i=1:size(tmp_metCompartment,2)
    if ~isempty(tmp_metCompartment) % in the case of an empty model
        ...
        tmp_compartment.annotation = makeSBMLAnnotationString_updata(model,tmp_compartment.metaid,'comp',i);
    end
...
if isfield(model,'genes')
    for i=1:length(model.genes)
        ...
        tmp_fbc_geneProduct.annotation = makeSBMLAnnotationString_updata(model,tmp_fbc_geneProduct.fbc_id,GeneProductAnnotations,i,true);
        if i==1
...
for i=1:size(model.rxns, 1)
    ...
    [tmp_Rxn.annotation,rxn_notes] = makeSBMLAnnotationString_updata(model,tmp_Rxn.metaid,'rxn',i);
    tmp_note = emptyChar;
